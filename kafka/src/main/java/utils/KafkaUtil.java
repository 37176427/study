...
public class KafkaUtil {
    ...
    public static final String BOOTSTRAP_SERVERS = "192.168.194.128:9092";
    ...
    public static KafkaProducer getKafkaProducer(){
        if(kafkaProducer == null){
             ...
             * 在消息负载不高的情况下，配置linger.ms能够让Producer在发送消息前等待一定时间，以积累更多的消息打包发送，达到节省网络资源的目的。默认为0
             */
    ...
    public static KafkaConsumer<String, String> getKafkaConsumer() {
        if(kafkaConsumer == null) {
            ...
            /**
             * group.id：Consumer的group id，同一个group下的多个Consumer不会拉取到重复的消息，不同group下的Consumer则会保证拉取到每一条消息。
